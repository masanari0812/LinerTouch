...
class KanaKeyboard:
    ...
    def on_canvas_drag(self, event=None):
        ...
        if self.is_dragging:
            ...
            offset_x_upper_limit = self.canvas_width / self.current_scale
            offset_y_upper_limit = self.canvas_height / self.current_scale
            offset_x_lower_limit = 0 / self.current_scale
            offset_y_lower_limit = 0 / self.current_scale
            self.offset_x += dx
            self.offset_y += dy
            # self.offset_x = max(offset_x_lower_limit, self.offset_x)
            # self.offset_x = min(offset_x_upper_limit, self.offset_x)
            # self.offset_y = max(offset_y_lower_limit, self.offset_y)
            # self.offset_y = min(offset_y_upper_limit, self.offset_y)
